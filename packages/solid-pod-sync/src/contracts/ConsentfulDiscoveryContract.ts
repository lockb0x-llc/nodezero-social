...
export const RELATIONSHIP_ACTIVITY_TYPES = ['Follow', 'Accept', 'Reject', 'Undo', 'Block'] as const
...
const MAX_DISCOVERY_MANIFEST_TTL_MS = 7 * 24 * 60 * 60_000
...
export function createDefaultDiscoveryConsent(
  ownerWebId: string,
  updatedAt: string
): DiscoveryConsent {
  return {
...
export function validateDiscoveryConsent(
  ...
  if (!isWebId(consent.ownerWebId))
    issues.push({
      field: 'ownerWebId',
      message: 'ownerWebId must be an https WebID with a fragment',
    })
  for (const key of DISCOVERY_CONSENT_KEYS) {
    if (typeof consent[key] !== 'boolean')
      issues.push({ field: key, message: `${key} must be a boolean` })
  }
  if (!isIsoDate(consent.updatedAt))
    issues.push({ field: 'updatedAt', message: 'updatedAt must be an ISO-compatible timestamp' })
  return issues
}
...
export function validateDiscoveryManifest(
  ...
  if (!isIsoDate(manifest.publishedAt))
    issues.push({
      field: 'publishedAt',
      message: 'publishedAt must be an ISO-compatible timestamp',
    })
  if (!isIsoDate(manifest.expiresAt))
    issues.push({ field: 'expiresAt', message: 'expiresAt must be an ISO-compatible timestamp' })
  if (
    isIsoDate(manifest.publishedAt) &&
    isIsoDate(manifest.expiresAt) &&
  ...
  if (
    ...
    isIsoDate(manifest.expiresAt) &&
    Date.parse(manifest.expiresAt) - Date.parse(manifest.publishedAt) >
      MAX_DISCOVERY_MANIFEST_TTL_MS
  ) {
    issues.push({ field: 'expiresAt', message: 'manifest lifetime cannot exceed seven days' })
  }
  if (manifest.displayName !== undefined && !manifest.displayName.trim())
    issues.push({ field: 'displayName', message: 'displayName cannot be blank' })
  if (manifest.avatarUrl !== undefined && !isHttpsUrl(manifest.avatarUrl))
    issues.push({ field: 'avatarUrl', message: 'avatarUrl must be an absolute https URL' })
  if (manifest.publicTypeIndexUrl !== undefined && !isHttpsUrl(manifest.publicTypeIndexUrl))
    issues.push({
      field: 'publicTypeIndexUrl',
      message: 'publicTypeIndexUrl must be an absolute https URL',
    })
  if (manifest.inboxUrl !== undefined && !isHttpsUrl(manifest.inboxUrl))
    issues.push({ field: 'inboxUrl', message: 'inboxUrl must be an absolute https URL' })
  if (!hasUniqueNonBlankValues(manifest.publicInterests))
    issues.push({
      field: 'publicInterests',
      message: 'publicInterests must contain unique non-blank strings',
    })
  if (!hasUniqueNonBlankValues(manifest.capabilities))
    issues.push({
      field: 'capabilities',
      message: 'capabilities must contain unique non-blank strings',
    })
  return issues
}
...
export function validateRelationshipActivity(
  activity: RelationshipActivity
): ConsentContractValidationIssue[] {
  const issues: ConsentContractValidationIssue[] = []
  if (activity.version !== 1) issues.push({ field: 'version', message: 'version must be 1' })
  if (!isHttpsUrl(activity.id))
    issues.push({ field: 'id', message: 'id must be an absolute https URL' })
  if (!RELATIONSHIP_ACTIVITY_TYPES.includes(activity.type))
    issues.push({ field: 'type', message: 'type is unsupported' })
  if (!isWebId(activity.actor))
    issues.push({ field: 'actor', message: 'actor must be an https WebID with a fragment' })
  if (!isWebId(activity.object) && !isHttpsUrl(activity.object))
    issues.push({ field: 'object', message: 'object must be an absolute https URL' })
  if (!isIsoDate(activity.publishedAt))
    issues.push({
      field: 'publishedAt',
      message: 'publishedAt must be an ISO-compatible timestamp',
    })
  if (
    (activity.type === 'Accept' || activity.type === 'Reject' || activity.type === 'Undo') &&
    !activity.inReplyTo
  ) {
    issues.push({
      field: 'inReplyTo',
      message: `${activity.type} must reference the activity it answers or undoes`,
    })
  }
  if (activity.inReplyTo !== undefined && !isHttpsUrl(activity.inReplyTo))
    issues.push({ field: 'inReplyTo', message: 'inReplyTo must be an absolute https URL' })
  return issues
}
...
export function validateRelationshipRecord(
  ...
  if (!isWebId(record.ownerWebId))
    issues.push({
      field: 'ownerWebId',
      message: 'ownerWebId must be an https WebID with a fragment',
    })
  if (!isWebId(record.peerWebId))
    issues.push({ field: 'peerWebId', message: 'peerWebId must be an https WebID with a fragment' })
  if (record.ownerWebId === record.peerWebId)
    issues.push({ field: 'peerWebId', message: 'peerWebId must differ from ownerWebId' })
  if (!RELATIONSHIP_STATES.includes(record.state))
    issues.push({ field: 'state', message: 'state is unsupported' })
  if (!isIsoDate(record.updatedAt))
    issues.push({ field: 'updatedAt', message: 'updatedAt must be an ISO-compatible timestamp' })
  if (record.activityId !== undefined && !isHttpsUrl(record.activityId))
    issues.push({ field: 'activityId', message: 'activityId must be an absolute https URL' })
  return issues
}
...
export function validateModerationRecord(
  ...
  if (!isWebId(record.subjectWebId))
    issues.push({
      field: 'subjectWebId',
      message: 'subjectWebId must be an https WebID with a fragment',
    })
  if (record.ownerWebId === record.subjectWebId)
    issues.push({ field: 'subjectWebId', message: 'subjectWebId must differ from ownerWebId' })
  if (!MODERATION_ACTIONS.includes(record.action))
    issues.push({ field: 'action', message: 'action is unsupported' })
  if (!isIsoDate(record.createdAt))
    issues.push({ field: 'createdAt', message: 'createdAt must be an ISO-compatible timestamp' })
  if (record.reasonCode !== undefined && !record.reasonCode.trim())
    issues.push({ field: 'reasonCode', message: 'reasonCode cannot be blank' })
  return issues
}
...
export function validateDeliveryReceipt(
  ...
  if (!isHttpsUrl(receipt.activityId))
    issues.push({ field: 'activityId', message: 'activityId must be an absolute https URL' })
  if (!isWebId(receipt.senderWebId))
    issues.push({
      field: 'senderWebId',
      message: 'senderWebId must be an https WebID with a fragment',
    })
  if (!isWebId(receipt.recipientWebId))
    issues.push({
      field: 'recipientWebId',
      message: 'recipientWebId must be an https WebID with a fragment',
    })
  if (!DELIVERY_STATUSES.includes(receipt.status))
    issues.push({ field: 'status', message: 'status is unsupported' })
  if (!isIsoDate(receipt.updatedAt))
    issues.push({ field: 'updatedAt', message: 'updatedAt must be an ISO-compatible timestamp' })
  if (receipt.errorCode !== undefined && !receipt.errorCode.trim())
    issues.push({ field: 'errorCode', message: 'errorCode cannot be blank' })
  return issues
}
...
export function validateProcessedActivityRecord(
  record: ProcessedActivityRecord
): ConsentContractValidationIssue[] {
  const issues: ConsentContractValidationIssue[] = []
  if (record.version !== 1) issues.push({ field: 'version', message: 'version must be 1' })
  if (!isHttpsUrl(record.activityId))
    issues.push({ field: 'activityId', message: 'activityId must be an absolute https URL' })
  if (!isWebId(record.actorWebId))
    issues.push({
      field: 'actorWebId',
      message: 'actorWebId must be an https WebID with a fragment',
    })
  if (!isIsoDate(record.processedAt))
    issues.push({
      field: 'processedAt',
      message: 'processedAt must be an ISO-compatible timestamp',
    })
  if (!isIsoDate(record.expiresAt))
    issues.push({ field: 'expiresAt', message: 'expiresAt must be an ISO-compatible timestamp' })
  if (
    isIsoDate(record.processedAt) &&
    isIsoDate(record.expiresAt) &&
    Date.parse(record.expiresAt) <= Date.parse(record.processedAt)
  ) {
