 ...
 *
 * ## `Lockb0x`
 * Maintains a ZK Merkle state root for Proof of Humanity.
 * The oracle updates this root after verifying off-chain ZK inclusion proofs.
 *
 * ## `PoHVerifier`
 * On-chain Groth16 ZK proof verifier.
 * Stores the Groth16 verification key (BN254) and verifies Proof-of-Humanity
 * proofs generated by the poh.circom circuit.  Records nullifiers on-chain
 * to prevent double-use.
 *
 * # ZK Stack
 *   - Circuit: poh.circom (circom 2.1.6, Groth16 on BN254)
 *   - Off-chain prover: @nodezero/zk-crypto (snarkjs)
 *   - On-chain verifier: PoHVerifier (ark-groth16 + ark-bn254, no_std)
 *
 * # Security notes
 ...
 * - `PoHVerifier` enforces nullifier uniqueness to prevent replay attacks.
 */
