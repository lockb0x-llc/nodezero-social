...
| ZK-07 | **Real ZK Pod-ownership attestation anchored on-chain** (identity commitment + Stellar-encrypted ciphertext) | ✅ Done | Replaces the sha256 "pairing root". `Lockb0x.set_attestation` stores `accountCommitment = Poseidon(identitySecret)` + AES-256-GCM encrypted claim ([lib.rs](../packages/contracts/src/lib.rs), new wasm `55bcb3a4…`). Device generates a `pod_ownership` Groth16 proof + encrypts the claim ([attestation.ts](../packages/mobile-app/src/onboarding/attestation.ts), [attestation-cipher.ts](../packages/zk-crypto/src/attestation-cipher.ts)); provisioner anchors it ([attestationAnchor.ts](../packages/jss-provisioner/src/attestationAnchor.ts)). **Live E2E PASS 2026-07-01** — lockbox `CBKUKJJFHUFWKB25NJBUVNH24HPO4JNNICWYYFZBHOINJMSR2PANVA7B`: on-chain `get_account_commitment=1aabc344…`, `get_attestation_ciphertext=0103361112…` (v1+nonce+AES-GCM), `storage_entries:5`, `attest` event. Off-chain login verify via [verifyLoginAttestation](../packages/zk-crypto/src/attestation-cipher.ts). **On-return fail-closed verify wired** (device `Poseidon(identitySecret)` vs on-chain `get_account_commitment` in [WalletContext.tsx](../packages/mobile-app/src/contexts/WalletContext.tsx)) + **WebID profile-card anchor slot** (`nz:lockboxContract`/`nz:stellarAccount`/`nz:accountCommitment` via [patchPodProfileAnchor](../packages/jss-provisioner/src/solidAccount.ts)) — both validated live 2026-07-01 (node `zkval9086356`) |
...
| Q-01 | Versioned contracts, Pod layout, discovery, relationship, moderation, and Type Index managers | ✅ Done | Public manifest, private relationship/moderation state, least-privilege ACLs, legacy state, and Type Index registration implemented; 18 suites/104 tests PASS, type-check PASS, lint zero errors |
...
| Q-03 | Reciprocal relationship lifecycle, legacy migration, mute/block/report, and `foaf:knows` projection | ✅ Done locally | Signed recipient-bound delivery and verification, default-off consent, bounded inbox sync/quarantine/cleanup, Follow/Accept/Reject/Undo, immutable retry, legacy import, accepted-only FOAF projection, Profile request UI, and compose block policy pass Solid 155, mobile 87, and provisioner 92 tests plus strict staging PWA artifact; deployment and Q1C abuse vectors remain separate gates |
