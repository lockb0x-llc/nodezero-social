...
| #   | Step                                                                       | Expected                                                                                                                                                   | Result          | Notes                                                                                                     |
| --- | -------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------- |
| QD1 | Create or migrate an account without changing discovery settings           | Directory listing, public indexing, nearby presence, inbound requests, and local broadcast participation are all off                                       | READY — NOT RUN | No consent is inferred from an old directory record, connection, Trust Circle, or OS location permission. |
| QD2 | Enable directory listing for the current account                           | A minimal public manifest is written to the owner's Pod and only that account appears in the derived index                                                 | READY — NOT RUN | A second account cannot mutate this manifest or index record.                                             |
| QD3 | Enable public indexing without directory listing                           | Selected public metadata is published but the account remains absent from Directory membership                                                             | READY — NOT RUN | Listing and indexing are independent.                                                                     |
| QD4 | Disable public indexing                                                    | Public interests, capabilities, and inbox metadata disappear while listing follows its independent setting                                                 | READY — NOT RUN | Failed replacement must not preserve stale public fields.                                                 |
| QD5 | Disable directory listing                                                  | The public index removes the account promptly while private profile, wallet, relationships, and Pod data remain usable                                     | READY — NOT RUN | Execute stale-device opt-out and rollback preservation cases.                                             |
| QR0 | Attempt a request while B's inbound-request consent is off, then enable it | Request is rejected while off and available only after explicit enablement                                                                                 | READY — NOT RUN | Directory membership does not imply inbound request consent.                                              |
| QR1 | User A sends User B a relationship request                                 | A sees outgoing pending; B sees incoming pending; neither is a directed audience recipient yet                                                             | READY — NOT RUN | Activity is durable, deduplicated, and correlated by immutable ID.                                        |
| QR2 | User B accepts the request                                                 | Both users see accepted state; compatible `foaf:knows` projection appears without deleting unrelated graph data                                            | READY — NOT RUN | Accepted and unblocked state enables directed communication.                                              |
| QR3 | Repeat with reject, cancel, and disconnect                                 | Each transition is idempotent and the UI converges after refresh/restart                                                                                   | READY — NOT RUN | No fabricated reciprocal history.                                                                         |
| QR4 | Disconnect an accepted relationship while remote delivery is unavailable   | Local accepted state and messaging authority are revoked immediately; failed notification remains retryable evidence                                       | READY — NOT RUN | A peer-controlled inbox cannot veto local revocation.                                                     |
| QS1 | User B blocks User A                                                       | A disappears from B's recommendations/actions and cannot reach B through LDN, compose, Waku, or relay fallback                                             | READY — NOT RUN | Block is private, purges rendered state, and overrides all transport state.                               |
| QN1 | Grant OS location while nearby presence remains off                        | H3 location can be computed locally but no presence beacon or nearby subscription begins                                                                   | READY — NOT RUN | Location and presence consent are independent.                                                            |
| QN2 | Enable nearby presence, reveal mutually, then revoke                       | Nearby peer appears without raw WebID; mutual reveal enables the shared profile/action flow; revoke stops publication/subscription and clears reveal state | READY — NOT RUN | Run revocation from the second physical device.                                                           |
| QC1 | Message an accepted contact from Directory, Profile, and Local             | All entry points show one relationship/safety state and deliver through the authorized channel                                                             | READY — NOT RUN | Unaccepted or blocked peers are rejected before send.                                                     |
| QA1 | Inspect public index, telemetry, and evidence                              | No private interests, Trust Circles, blocks, H3 history, reveal history, message content, credentials, or tokens appear                                    | READY — NOT RUN | Security and privacy release gate.                                                                        |
