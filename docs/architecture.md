...
2. Device derives a deterministic bootstrap password from a canonical
   onboarding message signed by the local Stellar keypair.
...
3. App calls provisioner `POST /v1/solid-account` (password is required;
   the password is deterministic bootstrap material derived on-device, not
...
8. If any bridge step fails, the login page falls back to manual
   credentials and forgot-password reset, so there is no dead end.
