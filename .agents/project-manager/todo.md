...
- [DONE] Q1C: Publish executable security test vectors for inbox ACLs, external fetch, replay, sender verification, privacy, migration, and block precedence. Owner: AUDIT_AGENT. Depends on Q0. Evidence: versioned manifest plus `pnpm policy:validate-consentful-discovery`; 22/22 vectors across 10/10 categories; actor-bound ETag-fenced replay leases, adversarial chunked inbox cancellation, retry-safe failures, complete embedded-private IPv6 denial, server Pod block policy, public-manifest sanitation, and delivery/verification rate limits; independent AUDIT_AGENT local GO.
- [DONE] Q2: Implement reciprocal relationship lifecycle, replay ledger, legacy `foaf:knows` migration, moderation state, and compatibility projection. Owner: SOLID_DATA_AGENT. Depends on Q1A/Q1B. Evidence: private consent/outbox/receipt/replay/quarantine stores; signed recipient-bound Follow/Accept/Reject/correlated Undo delivery; authenticated recipient verification; bounded inbox sync and cleanup; actor/recipient/time/replay/block checks; lazy legacy import; accepted-only FOAF projection preserving unrelated RDF; default-off Profile request controls; accepted-and-unblocked compose policy; immutable retry; Solid 168/168, mobile 90/90, provisioner 101/101 tests, strict staging PWA build/artifact, environment policy, and Q1C policy PASS. No deployment performed.
- [DONE] Q3A: Durable Azure Table projection storage, monotonic opt-out precedence, default-off keyed cohort flags, privacy-safe aggregate telemetry, exact-SHA relay deployment, retained 90-day rollback artifacts, and guarded retained-artifact rollback are complete locally with Azure/Audit GO. Provisioner 136/136, relay 3/3, consent policy 22/22, type/lint/env/PWA/YAML/diff gates PASS. Basic B1 does not support deployment slots; retained-artifact rollback is the approved local substitute pending P6 SKU/adoption approval. No Q3A deployment or rollback rehearsal has occurred.
- [DONE] Q3B: User consent controls, selected public interests, explainable recommendations, and unified Directory/Profile/Local actions are complete locally with Mobile/Audit GO. Deployment validation remains under Q3A/Q4.
- [DONE] Q3C: Consent, block, actor binding, abuse controls, and revocation are enforced across Presence, Waku, WebRTC, and relay paths locally with Mobile/Audit GO. Relay environment configuration and deployed journeys remain under Q3A/Q4.
- [IN_PROGRESS] Q4: Add consent policy validation, behavior-complete smoke, zero-retry two-account/browser/device coverage, deployment certification, rollback rehearsal, and soak. Owner: QA_RELEASE_AGENT. Q1-Q3 are locally complete; staging remains on `5d0d353`, so exact-SHA deployment provenance is the current blocker.
