...
- [IN_PROGRESS] Q3A: Add durable derived-index infrastructure, default-off flags, privacy-safe telemetry, retained slots/revisions, and rollback assets. Owner: AZURE_PLATFORM_AGENT. Depends on Q1 contracts and P6. API contract complete locally: session-authenticated owner refresh reads Pod consent before manifest, legacy internal-key mutation is retired, invalid/expired manifests remove stale projections, public fields/provenance are allowlisted, pages are bounded with cursors/ETags, and removal tombstones remain internal. Focused 18/18 and provisioner 113/113 tests PASS; durable Azure store/flags/slots/telemetry/rollback remain.
