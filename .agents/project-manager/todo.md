...
- [TODO] L1: Create new screen scaffolds: backpack.tsx, compose.tsx, docustream.tsx with mock state. Owner: MOBILE_APP_AGENT. (branch: agents/MOBILE_APP_AGENT/L1-ux-new-screens)
- [TODO] L2: Augment existing screens: feed.tsx (Algorithm Tuner modal), profile.tsx (Shared Threads + ZK badge), settings.tsx (remove WebACL section). Owner: MOBILE_APP_AGENT. (branch: agents/MOBILE_APP_AGENT/L2-ux-augment-screens)
- [TODO] L3: Navigation restructure (_layout.tsx) and npm dependency additions (@react-native-community/slider, react-native-rss-parser). Owner: MOBILE_APP_AGENT. (branch: agents/MOBILE_APP_AGENT/L3-ux-nav-deps)
- [TODO] L4: solid-pod-sync backend: updateWebACL(), findSemanticOverlap(), DocustreamManager.ts. Owner: SOLID_DATA_AGENT. (branch: agents/SOLID_DATA_AGENT/L4-ux-solid-backend)
- [TODO] L5: Context binding - wire Backpack/Docustream/Profile overlap to live backend methods. Owner: MOBILE_APP_AGENT. Blocked on L1+L2+L3+L4. (branch: agents/MOBILE_APP_AGENT/L5-ux-context-binding)
- [TODO] L6: Compose audience routing - local->P2PChannel, foaf->Pod encrypt, verified->ZK gate. Owner: P2P_RELAY_AGENT. Blocked on L1+L3. (branch: agents/P2P_RELAY_AGENT/L6-ux-compose-routing)
- [TODO] L7: Phase 3 QA smoke - J1(Backpack/WebACL) J2(Docustream/Pod) J3(Compose/P2P) J4(Semantic Overlap). Owner: QA_RELEASE_AGENT. Blocked on L5+L6. (branch: agents/QA_RELEASE_AGENT/L7-ux-qa-smoke)
