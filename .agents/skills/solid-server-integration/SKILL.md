...
If asked about a Solid server not covered in this skill, acknowledge the gap and offer to help based on general Solid specification knowledge, clearly stating that specific details for that server are not available here.
...
If the user has not specified a server, ask which server they are targeting before making recommendations. If interoperability across all servers is required, note the compatibility considerations for each option below.
